    ...
    if let Ok(RawWindowHandle::Win32(_w)) = handle.window_handle().map(|h| h.as_raw()) {
        match item {
            DragItem::Files(files) => {
                ...
                for f in files {
                    paths.push(plain_path(f.as_ref())?);
                }
            ...
            DragItem::Data { .. } => {
                ...
                let paths = vec![plain_path(Path::new("./"))?];
...
fn get_shell_item_array(paths: &[PathBuf]) -> Option<IShellItemArray> {
    unsafe {
        let list: Vec<*const Common::ITEMIDLIST> = paths
            ...
            .collect();
        // Windows が受け付けないパスは NULL になる。そのまま渡すと失敗するので、ここで諦める。
        if list.iter().any(|item| item.is_null()) {
            return None;
        }
...
/// ドラッグで渡すパスを絶対パスにする（Trayce の修正）。
///
/// 元は `dunce::canonicalize` だったが、ネットワークドライブ（`T:\`）を `\\?\UNC\server\...` に
/// 展開してしまい、`ILCreateFromPathW` がそれを受け付けずに NULL を返してアプリごと落ちていた。
/// ドライブ文字のまま絶対パスにし、存在しないものはエラーにする。
fn plain_path(path: &Path) -> std::io::Result<PathBuf> {
